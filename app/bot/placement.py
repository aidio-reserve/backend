...
def export_conversation_landmark(history):
    ...
    make_landmark_prompt = FewShotPromptTemplate(
        ...
        prefix="""
                UserとConciergeの会話の中で、現在Userが行きたい場所、地名、観光地名、場所名、ランドマーク、伝統工芸品、シンボルなどの名前が含まれている可能性がある。
                抽出できれば、それを出力しなさい。
                抽出できなければ、Noneを出力しなさい。
                ## 注意
                - Userの発言は、'User' で始まる。
                - Conciergeの発言は、'Concierge' で始まる。
                - それぞれの発言の最後には、発言に含まれる場所地名、観光地名、場所名、ランドマーク、伝統工芸品、シンボルなどの情報が記載された「location:」に続く文字列が提供されている。
                - 会話の履歴は最後の発言が最新である。
                - 最新の発言を優先して抽出する。
            """,
        suffix="会話履歴: {history}\n location:",
